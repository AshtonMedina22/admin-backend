  ...
  return (
    <Card className={cn("border-cyan-500/60 border-l-4", dashCardClass)}>
      ...
      <CardContent className={cn("space-y-4", dashCardContentClass)}>
        <div className="grid gap-2 rounded-lg border border-zinc-900 bg-zinc-950 p-3 font-mono text-xs text-zinc-400 leading-relaxed md:grid-cols-[10rem_1fr]">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {oauthScopeMap.map((item) => (
            <div key={item.scope} className="rounded-lg border border-zinc-900 bg-zinc-950/60 p-3">
      ...
      <CardContent className={dashCardContentClass}>
        <ol className="grid gap-2 text-muted-foreground text-xs leading-relaxed md:grid-cols-5">
          {websiteMaintenanceSteps.map((step, index) => (
            <li key={step} className="rounded-lg border border-zinc-900 bg-zinc-950/70 p-3">
      ...
      <CardContent className={dashCardContentClass}>
        <div className="grid gap-3 md:grid-cols-3">
          {automationRegistryRows.map((row) => (
            <div key={row.platform} className="rounded-lg border border-zinc-900 bg-zinc-950/70 p-3">
              <p className="font-semibold text-sm text-zinc-100">{row.platform}</p>
              <p className="mt-2 font-mono text-[11px] text-zinc-400 leading-relaxed">{row.flow}</p>
            </div>
          ))}
        </div>
      </CardContent>
      ...
      {domainMonitors.map((domain) => (
        ...
        >
          ...
          <CardContent className={cn("grid gap-2 text-sm", dashCardContentClass)}>
            <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-[#2a2a2d] bg-[#101113]/80 px-3 py-2">
              <span className="text-slate-400">SSL Handshake Status</span>
              <Badge variant={domain.critical ? "destructive" : "outline"} className="font-mono">
                {domain.ssl}
              </Badge>
            </div>
            <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-[#2a2a2d] bg-[#101113]/80 px-3 py-2">
              <span className="text-slate-400">Expiration / Renewal</span>
              <span className="font-medium font-mono">{domain.renewal}</span>
            </div>
            <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-[#2a2a2d] bg-[#101113]/80 px-3 py-2">
              <span className="text-slate-400">Assigned Administrator Handle</span>
  ...
  return (
    <Card className={dashCardClass}>
      ...
      <CardContent className={cn("grid gap-4", dashCardContentClass)}>
        <div className="scrollbar-none block w-full overflow-x-auto rounded-md border border-[#2a2a2d]">
        ...
        <div className="rounded-md border border-[#2a2a2d] bg-[#101113]/80 p-4 font-medium font-mono text-slate-300">
...
function GoogleWorkspaceArchitectureSpec() {
  ...
  return (
    <section className="mt-6 space-y-4 border-zinc-900 border-t pt-6">
      <div>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
        {specs.map((spec) => (
          <div
            key={spec.title}
            className="space-y-2 rounded-xl border border-zinc-900 bg-zinc-900/40 p-5 backdrop-blur-md"
          >
            <h3 className="font-bold font-mono text-cyan-400 text-xs">{spec.title}</h3>
            <p className="font-mono text-xs text-zinc-400 leading-relaxed">{spec.body}</p>
          </div>
        ))}
      </div>
    </section>
