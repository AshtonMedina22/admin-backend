...
function riskTextClass(status: PermitStatus) {
  return status === "critical" ? "text-red-600 dark:text-red-300" : "text-amber-700 dark:text-amber-300";
}
              ...
              <TableBody>
                {PERMITTING_QUEUE.map((row) => (
                  <TableRow key={row.permit} className={cn("h-11", rowAccentClass(row.brand))}>
                    ...
                    <TableCell
                      className={cn("py-2 text-right font-mono font-semibold tabular-nums", riskTextClass(row.status))}
                    >
                      {row.daysStale}
                    </TableCell>
