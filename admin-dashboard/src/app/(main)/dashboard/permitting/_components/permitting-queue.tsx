...
function brandBadgeClass(brand: string) {
  ...
  if (brand === "3SK") return "border-cyan-500/30 bg-white/5 text-cyan-700 dark:text-cyan-300";
        ...
        <div className="space-y-3">
          ...
          <ul className="space-y-2 text-muted-foreground text-xs leading-relaxed">
            {CONDITIONAL_FORMATTING_RULES.map((rule) => (
              <li key={rule} className="rounded-lg border border-zinc-900 bg-zinc-950/60 px-3 py-2">
                {rule}
              </li>
            ))}
          </ul>
        </div>
        <pre className="overflow-x-auto rounded-lg border border-zinc-900 bg-black/80 p-4 font-mono text-[11px] text-amber-200 leading-relaxed">
