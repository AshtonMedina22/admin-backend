...
function inferEntityBrand(text: string): EntityBrand {
    ...
    alerts:
      summary.alerts.length > 0
        ? summary.alerts.map((alert, index) => ({
            ...
            title: alert.alert,
            detail: alert.detail,
            company: alert.detail.toLowerCase().includes("solar2sk") ? "Solar 2SK" : "Shared",
            owner: "T. Khan",
            due: "Open",
...
export function mapRecordsToWebsiteSystems(records: SheetRecord[]): {
  ...
  const websites = records
    ...
    .map((row, index) => ({
      ...
      sslValid: row.ssl?.toLowerCase() !== "invalid",
  ...
  const subscriptions = records
    ...
    .map((row, index) => ({
      ...
      accountOwner: (row.account_owner?.includes("S.") ? "S. Khan" : "T. Khan") as SoftwareSubscription["accountOwner"],
