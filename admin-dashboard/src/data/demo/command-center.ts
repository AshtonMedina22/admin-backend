...
export type CommandCenterMetrics = {
  /** 3SK commercial contract + proposal pipeline ($). */
  b2bPipeline: number;
  /** YSP live macro fleet output (MW). */
  fleetYield: number;
  /** Combined installed macro capacity across managed sites (MW). */
  portfolioCapacity: number;
  /** 2SK MTD WooCommerce unit volume. */
  retailVolume: number;
};
...
export function computeCommandCenterMetrics(): CommandCenterMetrics {
  ...
  // YSP's broader managed/grid asset footprint in megawatts.
...
export const demoCommandCenterData: CommandCenterData = {
  ...
  kpis: [
    ...
    },
    {
      ...
      detail: "Pending 2SK backup kit orders",
