...
function MetricRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-4 rounded-md border border-[#dbe5ee] bg-[#ffffff]/80 px-3 py-2">
      <span className="text-slate-400 text-sm">{label}</span>
      <span className="font-medium text-slate-950 text-sm tabular-nums">{value}</span>
    </div>
  );
}
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <Card className={dashCardClass}>
          ...
          <CardContent className={cn("grid gap-3", dashCardContentClass)}>
            ...
            <div className="mt-2 rounded-md border border-[#dbe5ee] bg-[#f8fbff]/70 p-3 text-sm">
