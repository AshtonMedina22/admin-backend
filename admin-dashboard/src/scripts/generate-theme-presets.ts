 ...
 * - Run this script after updating theme tokens.
...
if (files.length === 0) {
  console.warn("Warning: No preset CSS files found. Only default preset will be included.");
}
...
const presets = files.map((file) => {
  ...
  if (!labelMatch) {
    console.warn(`Warning: No 'label:' found in ${file}, using filename as fallback.`);
  }
  if (!valueMatch) {
    console.warn(`Warning: No 'value:' found in ${file}, using filename as fallback.`);
  }
  ...
  if (!lightPrimaryMatch || !darkPrimaryMatch) {
    console.warn(`Warning: Missing --primary for ${file} (light or dark). Check CSS syntax.`);
  }
